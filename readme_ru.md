...
SalePrice имеет смещенное распределение, поэтому применяем логарифмирование для приведение к +- нормальному распределению
...
- `processed_reduced_numeric`
  - получем через выбор feature importance > `0.1`
- `nn_numeric_reduced`
...
Строим график для того чтобы понять визуально как отличается предсказанная цена от целовой. При увеличении размера слоев модель давала сильные выбросы на паре значений(порядка 2e6), что ухудшало результат. В целом на урезанном датасете модель ведет себя лучше
...
Лучший результат показал CatBooost

## 9. Идеи и доработки

Можно попытаться улучшить результаты путем работы с фичами. Например, считать возраст дома после ремонта; убрать часть скореллированных признаков(Есть бассейн и его качество; площадь гарада и число машин); сделать менее агрессивный срез по feature importance
